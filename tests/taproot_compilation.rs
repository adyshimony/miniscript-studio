...
// Test data - using the exact same data from the existing tests
...
fn test_taproot_modes_generate_different_addresses() {
    ...
    // CURRENT BUG: Script-path and single-leaf modes generate the same address
    ...
    // CURRENT BUG: These two generate the same address (this is what we need to fix)
...
fn test_taproot_script_path_mode_mainnet() {
    ...
    println!("✓ Script-path mainnet test passed");
    println!("  Address: {}", result.address);
}
